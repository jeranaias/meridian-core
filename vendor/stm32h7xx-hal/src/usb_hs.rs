...
macro_rules! usb_peripheral {
    ($USB:ident, $GLOBAL:ident, $en:ident, $rst:ident) => {
        ...
        unsafe impl UsbPeripheral for $USB {
            ...
            // MERIDIAN PATCH 2026-05-04: was `true`. On Cube Orange Plus
            // (and similar self-powered Pixhawk-style boards) USB routes
            // through OTG2 in FS-only mode on PA11/PA12 (AF10). With
            // HIGH_SPEED=true synopsys-usb-otg ran the HS init path and
            // left GCCFG.PWRDWN=0 (FS transceiver powered down) — USB
            // never enumerated. FS path sets PWRDWN=1 and applies the
            // F446/H7-style VBUS override automatically.
            // (USB1_ULPI keeps its own HS-mode UsbPeripheral impl below.)
